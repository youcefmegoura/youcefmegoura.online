...
export function V3Footer({ name, ui, t }: V3FooterProps) {
  const handleScrollTop = useCallback(() => {
    window.scrollTo({ top: 0, behavior: 'smooth' });
  }, []);

  return (
    <footer className="border-t border-zinc-800/50 py-8" role="contentinfo">
      <div className="mx-auto flex max-w-5xl flex-col items-center justify-between gap-4 px-5 sm:flex-row">
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 font-mono text-xs text-zinc-600 transition-colors hover:text-cyan-400"
